...
def with_object_name(value_expr: str, id_label: str, *, extra: str = "") -> str:
    ...
    **Join on `global_id`, NOT `object_id`.** `object_id` is the catalog item id
    ...
    `mz_object_info` is scoped to the selected environment(s): ids are only
    unique within an environment, so an unscoped join would match the same id
    across orgs in multi-tenant cloud. Every panel using this already filters its
    value side by env.
    """
    pulled = ["name", extra] if extra else ["name"]
    info = (
        f"label_replace({OBJECT_INFO}{{{variables.ENVIRONMENT_FILTER}}}, "
