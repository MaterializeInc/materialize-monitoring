...
- [Extending the schema: a worked example](../../../docs/content/reference/internal/pipelines/authoring.md#extending-the-schema-a-worked-example) — adding an attribute or a whole new component
...
**`loki.process` stages** (and `stage.match` body, recursively): `stage.match`, `stage.drop`, `stage.limit`, `stage.regex`, `stage.replace`, `stage.template`, `stage.logfmt`, `stage.json`, `stage.timestamp`, `stage.labels`, `stage.static_labels`, `stage.label_drop`, `stage.structured_metadata`, `stage.structured_metadata_drop`, `stage.sampling`. Each sub-block list always also accepts `raw:` as a final `oneOf` branch.

**Sub-blocks for `*.relabel`**: `rule` (shared between `loki.relabel` and `discovery.relabel`; defined once in `loki.schema.yaml`'s `$defs/ruleBlock`).
...
- **Phase 1d — Expression rendering**: `env` / `raw` / `function` / `ref` heads render; `operator` head is stubbed to error. The `expression` `$def` in `raw.schema.yaml` does **not** yet include `env`, and uses no `additionalProperties` discipline, so a YAML-authored expression is currently underspecified at the schema layer. Align once the AST shape is fully stable.
- **Phase 3b — Rust sugar deserialization**: typed blocks validate, but the `ComponentBlock` enum in `pipeline.rs` only has the `Raw(Block)` variant. Sugar variants (one per typed component) are needed before authored YAML can round-trip through the typed Rust path. Until then, typed blocks pass schema validation but the Rust deserializer would reject them; the existing `Pipeline::from_yaml_str` tests for typed blocks call `validate::validate` directly to side-step deserialization.

## Cleanup / refactor candidates

- **`Error::Render(String)`** is defined in `alloy/error.rs` but never constructed. Either delete or note its intended use.
- **`write_expression`** uses head-by-head `if let` + a `rendered_expr` flag. The idiomatic shape is a single tuple-match over `(&env, &raw, &function, &ref_name, &operator)`, which encodes "exactly one head set" structurally and removes the flag entirely. Documented for posterity in case it bites again.
...
- **`packages/alloy-pipelines/*.yaml` are placeholders**. Filling in the real gateway processor pipeline (port of `processor_pipeline.py`) is the next major authoring task. Typed schemas + `raw:` escape are ready for it.
