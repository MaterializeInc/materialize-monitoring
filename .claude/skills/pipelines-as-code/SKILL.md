...
| Log pipeline conventions, label families, multi-line records, retention | [Logging](../../../docs/content/reference/internal/pipelines/logging.md) |
...
**`loki.process` stages** (and `stage.match` body, recursively): `stage.match`, `stage.drop`, `stage.limit`, `stage.regex`, `stage.replace`, `stage.template`, `stage.logfmt`, `stage.json`, `stage.timestamp`, `stage.labels`, `stage.static_labels`, `stage.label_drop`, `stage.structured_metadata`, `stage.structured_metadata_drop`, `stage.sampling`, `stage.cri` (empty, no attributes), `stage.multiline` (`firstline` required; `max_lines`/`max_wait_time` `Expressable`, `trim_newlines` bool), `stage.tenant` (`label`/`source`/`value`, each `Expressable<String>`).
...
- **`stage.multiline`'s `firstline` is inverted from how it reads.** It matches the *start of a record*, and every line that does NOT match is appended to the record in progress — so it must match every ordinary line of the stream, and a panic's continuation lines are precisely what it must miss. Two consequences, both measured against a live install rather than reasoned about: (1) a stream whose format never matches is **unaffected** — entries pass through one at a time until the first match, so a wrongly-scoped container does not clump; the damaging case is a format that matches *intermittently*, where every line between two matches is swallowed. (2) `max_wait_time` is not a rare fallback but the actual delivery mechanism for a panic, because a panic is the last thing a process writes and nothing follows it to flush the block. Always scope the stage with `stage.match`, never apply it to a mixed stream.
