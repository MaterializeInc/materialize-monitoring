...
- **`process` is disabled and needs `hostPid: true`, not a capability.** It is the only source of `container_file_descriptors`, but reading another container's fd count means reading `/proc/<pid>/fd`. Without the host PID namespace cAdvisor still emits the series and reports **0** for every container but its own — measured 1/80 non-zero without `--pid=host` vs 19/80 with. Silently-zero is worse than absent, hence off by default.
