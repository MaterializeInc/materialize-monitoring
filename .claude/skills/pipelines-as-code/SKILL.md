...
| `packages/alloy-pipelines/agent.yaml` | `charts/.../pipelines/agent.alloy` | **implemented & largely typed** — staging-agent parity (journal + node-local pod logs → gateway, sampled debug tap), plus **node-local cAdvisor metrics** on a separate OTLP egress: `prometheus.exporter.cadvisor "local"` (raw) → typed `prometheus.scrape "cadvisor"` → typed `prometheus.relabel "cadvisor"` (maps `container_label_io_kubernetes_*` onto `namespace`/`pod`/`container`, adds `node`, drops the pod sandbox) → `otelcol.receiver.prometheus "cadvisorBridge"` (raw) → `otelcol.exporter.otlp "gateway"` (raw, `AGENT_OTLP_DEST`, default `alloy-gateway.<ns>.svc:4317`). Logs still leave via `loki.write`; folding them into the OTLP exporter is tracked separately |

Both pass `alloy validate`. `agent.yaml` is a faithful port of the reference `staging-agent.alloy` (`packages/ref-alloy-pipelines/`, not checked in). It started with several `raw:` escapes; most have since been graduated to typed forms (`stage.static_labels`/`selectors.field` via `Expressable`, `attach_metadata.namespace`, `stage.cri`). The `raw:` blocks that remain, all expected:
...
- `prometheus.exporter.cadvisor "local"` — **no `prometheus.exporter.*` is typed yet**; this is the first one in a pipeline. Deliberately raw during development, to be hardened into `prometheus.schema.yaml` once the argument set settles (queued).
- `otelcol.receiver.prometheus "cadvisorBridge"` / `otelcol.exporter.otlp "gateway"` — otelcol receivers and exporters are all raw-deferred (see the otelcol coverage note below); the `client`+`tls` sub-blocks come along for the ride.

**Agent log sidechannel** (`gateway.yaml`): the gateway tails the agent DaemonSet's own logs from the Kubernetes API — `discovery.kubernetes "agent_pods"` (typed, server-side `selectors.label = app.kubernetes.io/name=alloy-agent`) → `discovery.relabel "agent_pods"` (typed) → `loki.source.kubernetes "agent_logs"` (raw; not typed yet) → `loki.process.inputProcessor`. Points that are load-bearing rather than stylistic:
...
**cAdvisor specifics worth not re-deriving** (all verified by running the rendered config under a real `alloy run`, not just `alloy validate`):
- **Collector selection is env-driven and its precedence is blunt.** `ALLOY_CADVISOR_ENABLED` / `ALLOY_CADVISOR_DISABLED` are JSON arrays unwrapped with the same `encoding.from_json(coalesce(sys.env(…), "<default>"))` coercion used for `stage.limit` (here string→`list(string)` rather than string→int). A **non-empty** `enabled_metrics` replaces `disabled_metrics` outright rather than merging per collector — so enabled wins when a name is in both, but it is also the exhaustive list, not extras on top. Confirmed empirically: `ENABLED=["cpu","memory"]` + `DISABLED=["cpu"]` yields cpu and memory only, no network, no process.
- **No Linux capabilities are needed.** uid 0 with `capabilities.drop: [ALL]` (the agent's existing context) is sufficient for every collector we enable. The widespread "cAdvisor needs privileged/SYS_ADMIN" guidance is about the standalone DaemonSet on older runtimes.
- **`process` is ENABLED, and it is why the agent runs `controller.hostPID: true`.** It is the only source of `container_file_descriptors` / `container_ulimits_soft` (which the descriptor-exhaustion alerts divide against each other) plus `container_processes` / `container_sockets` / `container_threads`. Reading another container's fd count means reading `/proc/<pid>/fd`, so without the host PID namespace cAdvisor still emits every series and reports **0** for every container but the agent's own — measured 1/80 non-zero without `--pid=host` vs 19/80 with. Silently-zero is worse than absent, so **the collector and the flag move together**: disabling `hostPID` means adding `process` back to `disabledCollectors` in the same change. Measured cost of keeping it on: +5 series per container (~9% over cAdvisor's ~53) and ~+1ms scrape; flat per container, not per-CPU. The `/proc/<pid>/environ` exposure that `hostPID` + uid 0 grants is an accepted risk (DaemonSets here are privileged by default), and it changes no cluster's eligibility — the agent's hostPath mounts already put it outside the PSS *baseline* profile.
- **The alloy chart's key is `controller.hostPID`, not `hostPid`.** Our values carried `hostPid` on both Alloy roles for a long time; the chart never read it, so it was inert. Fixed on both. Watch for this shape — a plausible-looking but unread subchart key fails silently.
- **Host mounts are load-bearing, `/run/containerd` most of all.** Without it cAdvisor logs "Registration of the containerd container factory failed" and the `io.kubernetes.*` container labels never appear — which leaves the relabeling with nothing to map. The four CRI labels (`io.kubernetes.pod.{name,namespace,uid}`, `io.kubernetes.container.name`) are confirmed against a real containerd node.
- **The pod-sandbox drop rule is keyed on "has a pod but no container", not "container is empty".** If the label allowlist ever stops matching what the CRI sets, the simpler rule would silently drop *every* series; this one drops nothing, so the failure shows up as metrics without pod labels rather than as no metrics.

`gateway.yaml` is a port of the reference `staging-gateway.alloy` (`packages/ref-alloy-pipelines/`) — the fresher successor to `processor.alloy`. The `inputProcessor` block renders line-for-line against the reference (`sample_processor` deliberately omitted). Every stage is typed; the only `raw:` blocks are the ingress components (`loki.source.api`, `loki.source.kubernetes_events`, `otelcol.receiver.otlp`, `otelcol.exporter.loki`) and the `loki.write "destination"` sink — none of those are typed yet, so raw is expected (typing them, esp. otelcol's `otelcol.Consumer` capsule, is queued). Configurable knobs: `loki.source.api` `listen_port` via `ALLOY_LOKI_PORT` and the sink `url` via `GATEWAY_LOKI_DEST`, both `coalesce(sys.env(...), <default>)` (port also wrapped in the `encoding.from_json` int-coercion hack). **Egress is split into `gateway-dest-stub.yaml`**: `inputProcessor.forward_to` targets `loki.process.egress.receiver` (a type-neutral passthrough seam defined in the stub, which then forwards to `loki.write "destination"`), so the chart can swap the destination — including its component *type* (`loki.write` vs an `otelcol.receiver.loki` bridge vs a fan-out) — by rendering its own tail while keeping the `egress` label as the contract. This means `gateway.alloy` is **not** standalone-valid (dangling `loki.process.egress` ref); `make pipelines` validates it jointly with the stub (`cat gateway.alloy gateway-dest-stub.alloy | alloy validate /dev/stdin`) via a phony `alloy-pipelines-validate` target. The target *must* be a real component ref — a runtime env string can't name a receiver (`forward_to` is a `capsule`; alloy rejects a string at load, though `alloy validate` alone does *not* catch that — only a real load does). The ~11 identical mz-system service blocks (json → error/msg templates → header-extraction json ×2 → timestamp → structured_metadata) are written once via YAML anchors (`&mz_svc_*`) and aliased; `global-api-server` extends that shape with extra prehook/frontegg webhook stages. Anchors expand at parse time so the rendered `.alloy` is fully explicit. Deliberate divergences from the reference: the `cluster` fallback stays generic (`example_cluster`, not the staging-specific name) since the chart deploys everywhere; the sink drops the reference's `tenant_id`/`basic_auth`. Note: attribute *order* within a rendered block follows the typed sugar's `to_block` insertion order, not YAML/reference order — so `gateway.alloy` won't byte-match the reference inside `stage.drop`/`stage.regex`/`stage.replace`, but it's semantically identical (block attribute order is irrelevant to alloy).
...
**`prometheus.exporter.*` is not typed at all.** `prometheus.exporter.cadvisor` is the first one used in a pipeline (`agent.yaml`) and is a `raw:` block. Note `RelabelRule.replacement` is a plain `String`, not `Expressable` — a rule needing an env-derived replacement (e.g. stamping `node` from `HOSTNAME`) has to be a raw `rule` sub-block until that field is widened.
...
- **Type `prometheus.exporter.cadvisor`** (the stated plan: raw during development, schema after). Argument set is in `agent.yaml`; note `enabled_metrics`/`disabled_metrics` want `Expressable<Vec<String>>`-shaped handling, which the current `Expressable` deliberately does not cover (scalars only — a literal list would collide with the expression object). Either widen `Expressable`, or model the field as `anyOf: [list(string), expression]` in the schema with a hand-written deserializer.
- **`RelabelRule.replacement` as `Expressable<String>`**: currently a plain `String`, so stamping an env-derived label (`node` from `HOSTNAME` in `agent.yaml`) needs a raw `rule` sub-block. One-field change, removes a raw escape.
