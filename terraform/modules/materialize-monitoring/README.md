...
`examples/complete` is not a deployable root — its buckets and roles are placeholders. It exists as a plan target, so CI can compose the values and render the chart against them without a cluster:

```bash
cd examples/complete
terraform init -backend=false && terraform plan -out=tfplan
terraform show -json tfplan   # extract helm_release.monitoring.values, then helm template
```

That check has already earned its place twice: it caught a missing Loki NetworkPolicy selector, and two memcached StatefulSets that the scheduling fan-out was missing — both before any cluster was involved.
...
| <a name="input_chart_version"></a> [chart\_version](#input\_chart\_version) | Version of the materialize-monitoring chart. Pinned deliberately — bump it with the module, not independently. | `string` | `"0.8.0"` | no |
| <a name="input_crds_chart_version"></a> [crds\_chart\_version](#input\_crds\_chart\_version) | Version of the materialize-monitoring-crds chart. Tracked separately because the CRDs chart has a deliberately looser lifecycle. | `string` | `"0.3.0"` | no |
