...
* `materialize-monitoring/`
  ...
  * `bin/`: bash dev/CI entrypoints (flat; no subdirectories)
    ...
    * `check-alertmanager-config.sh`: render the chart's Alertmanager configuration per scenario and check it with `amtool check-config` (`make alertmanager-config-check`)
  ...
  * `packages/`: hand-authored, contributor-facing inputs
    ...
    * `queries/`: query-registry YAML inputs (`materialize-*.yaml` for the deployment, `infra-*.yaml` for the platform — including `infra-loki.yaml`, the log store watching itself — and `node-*.yaml` for node-exporter) — the metric/log/alert query definitions, validated against `mzmon-lib/schemas/query/mzmon-query.schema.yaml`
    * `alloy-pipelines/`: Alloy pipeline YAML inputs (`agent.yaml`, `gateway.yaml`, `gateway-metrics.yaml`, `gateway-dest-stub.yaml`)
    * `prometheus-scrapers/`: hand-authored scrape sources (`podmonitor-*.yaml`, `scrapeconfig-*.yaml`, `servicemonitor-*.yaml`) that the `scrape` transpiler renders into the per-flavor outputs under `pre-rendered/scrapers/`. The `podmonitor-cni-*.yaml` monitors are deployed to every cluster and self-disable on the ones running another vendor; see the [roadmap](/materialize-monitoring/preview/heather-alertmanager-prod/reference/internal/roadmap/#collection-gaps-these-depend-on)
  ...
  * `charts/`
    * `materialize-monitoring/`: umbrella chart
      ...
      * `tests/`: `helm-unittest` suites plus `__snapshot__/` (`make helm-tests`), and `alertmanager/` — scenario values for `make alertmanager-config-check`, which helm-unittest does not collect
    * `materialize-monitoring-dashboards/`: the Grafana dashboards, as a chart and release of their own — though not a *component* of their own: it belongs to the existing `dashboards` component, whose version stream it shares with `packages/dashboards/` and `packages/queries/`. Split out because Helm stores a release in a Kubernetes Secret and a Secret may not exceed 1 MiB, which the rendered set outgrew — the umbrella chart's `helm upgrade` began failing outright. It creates `GrafanaManifest` resources and nothing else; the `Grafana` instance, the datasources and the folders stay in the umbrella chart, and this one references them through values it cannot discover (`grafana.instanceSelector`, `grafana.folderUids`)
      * `pre-rendered/dashboards/`: `grafana/` and `datadog/`, written by `mz-monitoring-build gen-dashboards`
      * `templates/`: `dashboards.yaml` (the manifests) and `validate.yaml`
      * `values.yaml` / `README.md` / `README.md.gotmpl` / `tests/`
