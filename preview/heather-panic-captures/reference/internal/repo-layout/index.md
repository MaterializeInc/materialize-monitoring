...
* `materialize-monitoring/`
  ...
  * `packages/`: hand-authored, contributor-facing inputs
    ...
    * `prometheus-scrapers/`: hand-authored scrape sources (`podmonitor-*.yaml`, `scrapeconfig-*.yaml`) that the `scrape` transpiler renders into the per-flavor outputs under `pre-rendered/scrapers/`. The `podmonitor-cni-*.yaml` monitors are deployed to every cluster and self-disable on the ones running another vendor; see the [roadmap](/materialize-monitoring/preview/heather-panic-captures/reference/internal/roadmap/#collection-gaps-these-depend-on)
