...
>   See the [Dashboards workstream](/materialize-monitoring/preview/heather-roadmap-q4/reference/internal/roadmap/#dashboards) for where it sits against the rest of the work.

What exists today is the query material to build one yourself.

[Common Queries](/materialize-monitoring/preview/heather-roadmap-q4/reference/stable-metrics/common-queries/) renders every query in the registry with a **Datadog** tab beside its PromQL, in the metric query syntax you paste into a dashboard widget or a monitor.
...
The conventions behind them — how Prometheus metric names and labels land in Datadog, and where the two query languages diverge — are written up in [Datadog Translations](/materialize-monitoring/preview/heather-roadmap-q4/reference/internal/queries/datadog/) (internal).
...
See [Storing Metrics](/materialize-monitoring/preview/heather-roadmap-q4/metrics/storing/#datadog) for the exporter, and the `otel-metrics-fanout` profile for the assembled shape.
