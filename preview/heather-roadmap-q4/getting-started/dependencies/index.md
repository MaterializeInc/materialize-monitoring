...
Almost everything on this page is the `[consumer]` half of the [shared responsibility model](/materialize-monitoring/preview/heather-roadmap-q4/operating/production-best-practices/#shared-responsibility-model): cloud resources, secrets, and cluster facilities the chart consumes **by name** but never creates.
...
> [!TIP]
>  Installing with the [Terraform modules](/materialize-monitoring/preview/heather-roadmap-q4/getting-started/terraform/)? They create most of this for you — buckets, identity bindings, the Grafana database, the load balancer.
...
See [Compatibility](/materialize-monitoring/preview/heather-roadmap-q4/reference/compatibility/).
...
Pick a [sizing profile](/materialize-monitoring/preview/heather-roadmap-q4/getting-started/helm/#sizing) that matches the cluster you actually have.
...
[Custom Resource Definitions](/materialize-monitoring/preview/heather-roadmap-q4/reference/crds/) is the full inventory — every kind, what the stack does with each, how to disable the ones you do not need, and the two groups (Google Managed Prometheus, cert-manager) that this chart deliberately does not install.
...
* [Logs & Events → Storing](/materialize-monitoring/preview/heather-roadmap-q4/logs-and-events/storing/#granting-object-storage-access-workload-identity) — IRSA, GKE Workload Identity, and Entra Workload ID
* [Metrics → Storing](/materialize-monitoring/preview/heather-roadmap-q4/metrics/storing/#authentication) — the same three for Thanos
...
>  See [Selecting the backend](/materialize-monitoring/preview/heather-roadmap-q4/logs-and-events/storing/#selecting-the-backend).
...
Version requirements are in [Compatibility](/materialize-monitoring/preview/heather-roadmap-q4/reference/compatibility/). In short: the scrapers need the `environmentd` labels introduced in Materialize **v26.24.0**, and the dashboards want the `mz_object_info` metric introduced in **v26.29.0** — without it they degrade gracefully rather than break.
...
See [Scraping](/materialize-monitoring/preview/heather-roadmap-q4/metrics/scraping/#authenticating-the-sql-metrics-endpoint), and [Securing](/materialize-monitoring/preview/heather-roadmap-q4/operating/securing/#materialize-metrics-endpoint) for the access-control consequence.
...
See [Securing](/materialize-monitoring/preview/heather-roadmap-q4/operating/securing/#certificates).
...
See [Securing](/materialize-monitoring/preview/heather-roadmap-q4/operating/securing/) for what each exposure combination is graded as.
...
Apply `profiles/no-zone-spread.values.yaml` — see [Production Best Practices](/materialize-monitoring/preview/heather-roadmap-q4/operating/production-best-practices/#thanos-few-zones).
...
See [Container metrics from the kubelet](/materialize-monitoring/preview/heather-roadmap-q4/metrics/scraping/#kubelet).

## If you install with Terraform {#terraform}

The [Terraform modules](/materialize-monitoring/preview/heather-roadmap-q4/getting-started/terraform/) cover the cloud-resource dependencies above: buckets, the IAM role or service account with its workload-identity binding, the Grafana database, an internal load balancer, and the CRDs chart.
...
* **Rolling Alloy after a pipeline or filter change.** The module hashes the composed values to do this for you; on the Helm path it is yours, and it is the one place the chart cannot own its own rollout. See [Production Best Practices](/materialize-monitoring/preview/heather-roadmap-q4/operating/production-best-practices/#collection-alloy).

## Next steps

Once the dependencies are in place: [Installing via Terraform](/materialize-monitoring/preview/heather-roadmap-q4/getting-started/terraform/) or [Installing via Helm](/materialize-monitoring/preview/heather-roadmap-q4/getting-started/helm/).
