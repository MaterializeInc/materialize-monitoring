...
They are authored the same way as log pipelines — see [Authoring](/materialize-monitoring/preview/claude-secret-filter-log-proposal-kkw9kt/reference/internal/pipelines/authoring/) for the pipeline model, the strict-attributes policy, and the `raw:` escape hatch.
...
> Load-testing note: `alloy validate` does not catch capsule-type mismatches (see [Authoring](/materialize-monitoring/preview/claude-secret-filter-log-proposal-kkw9kt/reference/internal/pipelines/authoring/)).
