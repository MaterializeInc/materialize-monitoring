...
> [!TIP]
>  Installing with the [Terraform modules](/materialize-monitoring/preview/claude-enable-thanos-loki-rulers-d84e91/getting-started/terraform/)? They create most of this for you — buckets, identity bindings, the Grafana database, the load balancer.
...
See [Compatibility](/materialize-monitoring/preview/claude-enable-thanos-loki-rulers-d84e91/reference/compatibility/).
...
Pick a [sizing profile](/materialize-monitoring/preview/claude-enable-thanos-loki-rulers-d84e91/getting-started/helm/#sizing) that matches the cluster you actually have.
...
* [Logs & Events → Storing](/materialize-monitoring/preview/claude-enable-thanos-loki-rulers-d84e91/logs-and-events/storing/#granting-object-storage-access-workload-identity) — IRSA, GKE Workload Identity, and Entra Workload ID
* [Metrics → Storing](/materialize-monitoring/preview/claude-enable-thanos-loki-rulers-d84e91/metrics/storing/#authentication) — the same three for Thanos
...
>  See [Selecting the backend](/materialize-monitoring/preview/claude-enable-thanos-loki-rulers-d84e91/logs-and-events/storing/#selecting-the-backend).
...
See [Securing](/materialize-monitoring/preview/claude-enable-thanos-loki-rulers-d84e91/operating/securing/#certificates).
...
See [Securing](/materialize-monitoring/preview/claude-enable-thanos-loki-rulers-d84e91/operating/securing/) for what each exposure combination is graded as.
...
Apply `profiles/no-zone-spread.values.yaml` — see [Production Best Practices](/materialize-monitoring/preview/claude-enable-thanos-loki-rulers-d84e91/operating/production-best-practices/#thanos-few-zones).
...
See [Container metrics from the kubelet](/materialize-monitoring/preview/claude-enable-thanos-loki-rulers-d84e91/metrics/scraping/#kubelet).

## If you install with Terraform {#terraform}

The [Terraform modules](/materialize-monitoring/preview/claude-enable-thanos-loki-rulers-d84e91/getting-started/terraform/) cover the cloud-resource dependencies above: buckets, the IAM role or service account with its workload-identity binding, the Grafana database, an internal load balancer, and the CRDs chart.
...
* **Rolling Alloy after a pipeline or filter change.** The module hashes the composed values to do this for you; on the Helm path it is yours, and it is the one place the chart cannot own its own rollout. See [Production Best Practices](/materialize-monitoring/preview/claude-enable-thanos-loki-rulers-d84e91/operating/production-best-practices/#collection-alloy).

## Next steps

Once the dependencies are in place: [Installing via Terraform](/materialize-monitoring/preview/claude-enable-thanos-loki-rulers-d84e91/getting-started/terraform/) or [Installing via Helm](/materialize-monitoring/preview/claude-enable-thanos-loki-rulers-d84e91/getting-started/helm/).
