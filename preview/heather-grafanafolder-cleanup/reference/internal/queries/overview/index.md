...
are end-user voice; see [Dashboard Style Guidelines](/materialize-monitoring/preview/heather-grafanafolder-cleanup/reference/internal/dashboard/style-guidelines/) for that.

## In this section

- **[Datadog Translations](/materialize-monitoring/preview/heather-grafanafolder-cleanup/reference/internal/queries/datadog/)** — the PromQL→Datadog mapping, the OTLP naming assumptions it
...
| `datadogQuery` | Datadog | Metric query syntax, not DDSQL. See [Datadog Translations](/materialize-monitoring/preview/heather-grafanafolder-cleanup/reference/internal/queries/datadog/). |
...
- **Grafana dashboards** take both their PromQL *and* their panel descriptions from the registry, through
  ...
  [Dashboards → SDKs and Schemas](/materialize-monitoring/preview/heather-grafanafolder-cleanup/reference/internal/dashboard/sdks/#panels-do-not-write-promql).
...
- **Metric extraction** (`mz-monitoring-build extract-metrics`) parses the PromQL to derive the metric set, which lands
  ...
  [Reference Metrics](/materialize-monitoring/preview/heather-grafanafolder-cleanup/reference/stable-metrics/list-metrics/).
...
- **The docs** read `packages/queries/` directly: Hugo mounts it at `assets/queries/`, and the `list-queries` shortcode
  renders [Common Queries](/materialize-monitoring/preview/heather-grafanafolder-cleanup/reference/stable-metrics/common-queries/) from it.
