...
For the install procedure in context, see [Dependencies](/materialize-monitoring/preview/heather-garage/getting-started/dependencies/) and [Installing via Helm](/materialize-monitoring/preview/heather-garage/getting-started/helm/).
...
>  Removing them is a separate, explicit step — see [Uninstalling](/materialize-monitoring/preview/heather-garage/operating/uninstalling/).
...
See [Scraping](/materialize-monitoring/preview/heather-garage/metrics/scraping/#classic) for the downloads.
...
Progress is tracked in the [Rules & alerts](/materialize-monitoring/preview/heather-garage/reference/internal/roadmap/#rules--alerts) workstream.
...
| `GrafanaFolder` | **Created.** The folders the bundled dashboards are filed under — see [Folders](/materialize-monitoring/preview/heather-garage/dashboards/grafana/grafana-operator/#folders) |
...
See [Grafana Operator](/materialize-monitoring/preview/heather-garage/dashboards/grafana/grafana-operator/) and [Grafana Architecture](/materialize-monitoring/preview/heather-garage/dashboards/grafana/architecture/).
...
**Google Managed Prometheus.** The repo ships `PodMonitoring` and `ClusterPodMonitoring` artifacts for the GMP path, but no template installs them — they are downloads for a GMP consumer, the same way the classic `ScrapeConfig` is. See [Scraping](/materialize-monitoring/preview/heather-garage/metrics/scraping/#gmp).
...
[o11y Troubleshooting](/materialize-monitoring/preview/heather-garage/operating/o11y-troubleshooting/) covers that path.
