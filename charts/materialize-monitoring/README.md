...
| `certFile` / `keyFile` | The serving certificate. The defaults are the chart's `mzmon-alertmanager-tls` Secret, which `alertmanager.extraSecretMounts` mounts at `/etc/mzmon/tls` whether or not it exists yet. |
...
URL and, when `alerting.server.tls` is on, the CA and client certificate, so
`kubectl exec ... -- amtool silence query` works unchanged in every TLS phase.
