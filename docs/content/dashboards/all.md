...
| **Grafana folder** (`grafana.app/folder`) | The UID of the folder the dashboard is filed under. It is Grafana's own annotation rather than one of ours, and the chart creates the folder it names — see [Folders]({{< relref "grafana/grafana-operator.md" >}}#folders). On a hand-imported dashboard, the folder is whichever one you import into, and this is ignored. |
