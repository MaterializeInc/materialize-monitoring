...
#### Dashboard delivery is at the 1 MiB ceiling

**The default dashboard set no longer fits in a Helm release.**
Helm stores each release as a Kubernetes Secret, and a Secret's data may not exceed 1 MiB.
Adding a seventh dashboard crosses it: on a reference install the release measured 1,012,374 bytes with six, leaving
36,202 bytes of headroom, and `infra-loki` needs more than that.
The failure is `Secret "sh.helm.release.v1.<release>.vN" is invalid: data: Too long`, raised on `helm upgrade` before
anything is applied — so it is safe, loud, and completely blocking.

This is the same 1 MiB object ceiling the Terraform design doc already
[flagged for the sidecar path](../design-docs/20260803-terraform-modules/), reached from the other direction.
That note assumed the grafana-operator path had room; it does not any more.

The constraint is the *total* of the selected dashboards, not any one of them.
`infra-loki` renders to 162 KB, which is mid-range — `infra-nodes` is 246 KB and `env-top` 335 KB.
Six dashboards including `infra-loki` install cleanly today; the seven that `["env-*", "infra-*"]` selects do not.

None of the obvious mitigations is free, which is why this is a decision rather than a fix:

| Option | Cost |
|---|---|
| Narrow the default `dashboards.selected` | A shipped default becomes narrower, and which dashboard loses is arbitrary |
| Split dashboards into their own Helm release | A second release to version, upgrade and delete in step with the first |
| Shrink the rendered output | Real headroom — the YAML is verbose — but bounded, and it buys one or two dashboards rather than a policy |
| Deliver dashboards outside the release entirely | The largest change, and the only one that stops the problem recurring |
