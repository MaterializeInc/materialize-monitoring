...
* `materialize-monitoring/`
  ...
  * `charts/`
    ...
    * `materialize-monitoring-crds/`: CRDs chart (`Chart.yaml`, `values.yaml`, `README.md`)
      * `charts/grafana-operator-crds/`: Grafana Operator CRDs deflated from the upstream operator chart, which publishes no CRDs chart of its own — generated by `bin/extract-grafana-operator-crds.sh` (`make grafana-operator-crds`)
