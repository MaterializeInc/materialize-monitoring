...
>
>   **A fourth, found when the gateway's own listeners moved to Helm: `min_version` has three vocabularies.** Client blocks take `TLS13`, the dskit listeners take `VersionTLS13`, and `otelcol.receiver.otlp` takes `1.3`. The dskit mismatch crashloops the pod; **the otelcol mismatch is silent** — the component goes unhealthy, the listener never binds, and alloy keeps running with the pod Ready. `alloy validate` catches none of them, and neither does pod status; the component-health endpoint (`/api/v0/web/components` on 12345) is the only thing that shows it. Any future work here should check component health rather than "did it start".
>
...
| Typed `tls` blocks on `loki.source.api`, `otelcol.receiver.otlp`, **and `prometheus.receive_http`** in the Alloy **schema**. All three now render TLS from Helm (`mzmon.alloyGateway.pipeline.sources`) and are verified authenticated on a live cluster, but the Helm path bypasses the JSONSchema — only the pre-install `alloy validate` job checks them | — | 1 |
| ~~`pipeline.logging.gateway.server.tls` **and `pipeline.metrics.gateway.server.tls`** values and rendering~~ **Shipped.** `prometheus.receive_http` moved out of the pre-rendered metrics pipeline into the Helm-rendered sources, which is what made the metrics ingest port configurable at all. Every gateway ingress port — 3100, 4317, 4318, 9090 — refuses a client with no certificate at phase 3, verified on a live cluster | above | 1 |
