...
| Variable | Default | Notes |
|---|---|---|
| `storage_class` | `null` (cluster default) | Reaches the four PVC-backed workloads — Alertmanager, the Loki ruler, the Thanos Store Gateway and Compactor. **Required on GCP C4/N4 node pools** — see below. Loki's ingesters and Thanos Receive are unaffected; they use node-local `emptyDir` by design |
